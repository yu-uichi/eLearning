...
//GETでトークンの埋め込み
//URLは変更されます．
$url = "https://kokushi.teamdigicom.net/password_reset.php"."?urltoken=".$urltoken;

//メンバーの仮登録
$insert_result = insertResetMember($mail, $urltoken);
$message = '';
...
if ($insert_result == TRUE) {
    ...
    $mail_result = "このメールアドレスのユーザ情報がありません";
}

//メンバーの仮登録
function insertResetMember($mail, $urltoken) {
    $dbh = connectDB();
    if (!$dbh) {
    ...
    if ($result == FALSE) { //メンバーになっていない
        return FALSE;
    }

    $sql = 'INSERT INTO `reset_member_tb` (`urltoken`, `mail`, `date`)
...
function sendMail($mail, $url) {
    ...
    $subject = '[' . $web_name . '] パスワードリセットのお知らせ';
