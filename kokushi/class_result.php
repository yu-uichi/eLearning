...
<?php
  ...
  $sql = "SELECT `id` FROM `affiliation_tb` WHERE `affiliation` = '". $_SESSION['affiliation'] ."'";//affiliationIDの取得
  $sth = $dbh->query($sql); //SQLの実行
  $affiliation_id = $sth->fetch()['id'];
  $sql = "SELECT `id`,`family_name`,`given_name`,`student_id` FROM `user_tb` WHERE `teacher`=0 AND `affiliation_id` = '".$affiliation_id."'";//教員と同じ大学の学生IDと学籍番号の取得
  $sth = $dbh->query($sql); //SQLの実行
...
<div class="container">
<form action="">
    <select name="" id="" data-placeholder="生徒を選択" class="chosen-select"　tabindex="2">
        <option value="-1"></option>
        <option value="-1">全生徒</option>
        <?php
            $id_list;
            //while($row = $sth->fetch()){
                //$id_list[]=$row['id'];
                //echo "<option value='".$row['id']."'>".$row['student_id']."  ".$row['family_name']." ".$row['given_name']."</option>";
            //}
...
<script>
    $(".chosen-select").chosen();//dropdown検索用

    periodChart('year',$('select').val());
    $('#column-tab').on('click', function() {
      $('#fst-label').text('全期間');
      periodChart($('input[name="periodRadio"]:checked').val(),$('select').val());
    });

    $('#line-tab').on('click', function() {
      lineChart($('input[name="periodRadio"]:checked').val(),$('select').val());
      $('#fst-label').text('今年');
    });

    $( 'input[name="periodRadio"]:radio' ).change( function() {
      if($('#column').hasClass('active')){
        periodChart($(this).val(),$('select').val());
      }else{
        lineChart($(this).val(),$('select').val());
      }
    ...
    $('select').change(function() {
      if($('#column').hasClass('active')){
        periodChart($('input[name="periodRadio"]:checked').val(),$(this).val());
      }else{
        lineChart($('input[name="periodRadio"]:checked').val(),$(this).val());
      }
    });



    function periodChart(periodType,userId){
        idList=<?php echo json_encode($id_list);?>;
        $.ajax({
            type: "POST",
            url: 'class_result_ajax.php',
            dataType:"json",
            data: {
    ...
    function lineChart(lineType,userId){
            idList=<?php echo json_encode($id_list);?>;
            $.ajax({
            type: "POST",
            url: 'class_result_line_ajax.php',
            dataType:"json",
            data: {
                'lineType': lineType,
                'userId': userId,
